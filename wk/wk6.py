...
#
# # def k_fn(X1, X2):
# #     temp = np.sqrt((np.sum(X1*X1,1)[:,None]) + np.sum(X2*X2,1)[None,:] - np.dot(X1,(2*X2.T)))
# #     return (1+temp)*(np.exp(-temp))
#
...
X = np.array([[1, 1, 3], [2, 6, 4], [3, 4, 4],[3, 1, 4]])
def h1(X):
    ...
    """
    w11 = np.array([0 if i != 0 else -1 for i in range(X.shape[1])])
    w12 = np.array([0 if i != 1 else 1 for i in range(X.shape[1])])
    b1 = 0
    ww1 = np.array([w11, w12])
    # `aa` is a linear combination of the inputs
    aa = np.sum(X @ ww1.T, axis=1) + b1
